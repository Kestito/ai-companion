...
# Fixed: Resolved the 'coroutine' object has no attribute 'get' error
# The issue was in helpers.py where memory_service.load_memory_to_graph was called
# without the 'await' keyword. This was causing a coroutine to be returned instead
# of the actual result, which then caused errors when trying to access attributes.
...
async def process_message(request: MessageRequest) -> MessageResponse:
    ...
    try:
        ...
        response = result.get(
            "output_message", "Sorry, I couldn't process your message."
        )
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        while True:
            ...
            await websocket.send_json({"response": response, "session_id": session_id})
        ...
        await websocket.send_json(
            {"error": f"Error processing message: {str(e)}", "session_id": session_id}
        )
