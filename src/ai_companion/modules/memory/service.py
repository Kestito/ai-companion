...
class MemoryService:
    ...
    async def get_session_memory(
        ...
        try:
            ...
            cached_messages = await self.short_term_memory.get_cached_messages(session_id, limit)
            
            if cached_messages:
                logger.debug(f"Retrieved {len(cached_messages)} cached messages for session {session_id}")
                        ...
                        for item in result.data:
                            try:
                                ...
                                if "conversation" in context:
                                    ...
                                    messages.append({
                                        "id": item.get("id"),
                                        "content": conversation.get("user_message", ""),
                                        "response": conversation.get("bot_response", ""),
                                        "metadata": metadata,
                                ...
                                logger.warning(f"Error parsing memory from database: {e}")
                                continue
                        
                        logger.debug(f"Retrieved {len(messages)} memories from database for session {session_id}")
                        return messages
                except Exception as e:
        ...
        try:
            ...
            if self.supabase:
                try:
                    ...
                    logger.info(f"Stored complete memory state in Supabase with ID: {memory_id}")
                except Exception as e:
    ...
    async def load_memory_to_graph(
        ...
        try:
            ...
            if query:
                ...
                memory_context = self.long_term_memory.format_memories_for_prompt(relevant_memories)
            
            # Create initial state with memory context
            state = {"messages": config.get("messages", []), "memory_context": memory_context}
            ...
            if self.supabase:
                try:
                    ...
                    if result.data and len(result.data) > 0:
                        ...
                        logger.info(f"Retrieved existing state from Supabase for session {session_id}")
                    ...
                    logger.warning(f"Error retrieving existing state from Supabase: {e}")
