...
class ShortTermMemoryManager:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning("Falling back to in-memory store after table initialization failure")
    ...
    def _init_table(self):
        ...
        try:
            ...
            try:
                logger.warning(f"Table {self.table_name} not found, attempting to check if it needs to be created")
                
                # Check if this is a 'table does not exist' error
                if isinstance(e, dict) and e.get('code') == '42P01':
                    logger.warning(f"The table {self.table_name} does not exist, will use in-memory storage only")
    ...
    async def _sync_to_database(self):
        ...
        if tasks:
            ...
            logger.debug(f"Successfully synced {successful_syncs}/{len(tasks)} sessions to database")
    
    async def _sync_session_to_database(self, session_id: str, messages: List[Dict[str, Any]]) -> bool:
        """
        ...
        try:
            ...
            for message in messages:
                ...
                expires_at = datetime.fromtimestamp(message.get("expires_at", 0)).isoformat()
            ...
            if hasattr(result, "data"):
                ...
                logger.warning(f"Sync failed for session {session_id}: No data returned")
                return False
    ...
    async def get_active_memories(self) -> List[Memory]:
        ...
        if self.supabase and self.table_exists:
            try:
                ...
                result = self.supabase.table(self.table_name).select("*").gt("expires_at", now).execute()
    ...
    async def get_cached_messages(self, session_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        Get cached messages for a session (fast in-memory retrieval).
        ...
        """
        return await self.cache.get_messages(session_id, limit)
    ...
    async def get_messages_parallel(
        ...
        unique_db_messages = [msg for msg in db_messages if msg.get("id") not in cache_ids]
    ...
    async def store_with_cache(
        ...
        """
        # Ensure metadata has session_id
    ...
    async def add_memory(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> Memory:
        """
        Add a new memory.
