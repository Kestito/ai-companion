...
async def test_embedding():
    ...
    try:
        ...
        logger.info(
            f"Embeddings deployment parameter: {settings.AZURE_EMBEDDING_DEPLOYMENT}"
        )
        ...
        if hasattr(result, "embeddings"):
            logger.info(
                f"Embeddings attribute exists with length: {len(result.embeddings)}"
            )
            if result.embeddings:
                logger.info(f"First embedding dimensions: {len(result.embeddings[0])}")
        else:
            logger.info(
                f"Result has no 'embeddings' attribute. Available attributes: {dir(result)}"
            )
