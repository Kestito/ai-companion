...
class URLTracker:
    ...
    def _load_processed_urls(self):
        ...
        try:
            if os.path.exists(self.storage_file):
                ...
                logger.info(
                    f"Loaded {len(self.processed_urls)} processed URLs from storage"
                )
            else:
    ...
    async def mark_url_processed(
        self, url: str, status: str = "success", metadata: dict = None
    ):
...
class ProcessedChunk:
    ...
    def __post_init__(self):
        ...
        if len(self.embedding) != VECTOR_SIZE:
            raise ValueError(
                f"Embedding dimension mismatch: {len(self.embedding)} != {VECTOR_SIZE}"
            )
...
async def get_title_and_summary(
    chunk: str, url: str, db: DatabaseClients
) -> Dict[str, str]:
    """Extract title and summary using o4-mini with rate limiting."""
    ...
    try:
        ...
        if not result.get("title") or not result.get("summary"):
            ...
            result["title"] = (
                result.get("title")
                or f"Dokumentas iš {url.split('/')[-2].replace('-', ' ').title()}"
            )
...
async def get_embedding(text: str, db: DatabaseClients) -> List[float]:
    ...
    try:
        ...
        if len(embedding) != VECTOR_SIZE:
            raise ValueError(
                f"Embedding size mismatch: got {len(embedding)}, expected {VECTOR_SIZE}"
            )
...
async def get_semantic_context(
    chunk: str, title: str, summary: str, url: str, db: DatabaseClients
) -> str:
    """Generate semantic context for a chunk using o4-mini."""
    await completions_limiter.acquire()
    ...
    try:
        ...
        # Log the attempt
        logger.info(
            f"Generating semantic context for chunk from {url} (cache_key: {cache_key[:8]})"
        )

        # Validate inputs
        if not chunk or not title or not summary:
            logger.warning(
                f"Missing required input for semantic context generation: chunk={bool(chunk)}, title={bool(title)}, summary={bool(summary)}"
            )
            return ""
        ...
        logger.info(
            f"Successfully generated semantic context for chunk from {url} (cache_key: {cache_key[:8]})"
        )
        return semantic_context

    except Exception as e:
        logger.error(
            f"Error getting semantic context for {url}: {str(e)}", exc_info=True
        )
...
async def process_chunk(
    chunk: str, chunk_number: int, url: str, db: DatabaseClients
) -> ProcessedChunk:
    ...
    try:
        ...
        if not title_summary:
            logger.warning(
                f"Failed to get title and summary for chunk {chunk_number} from {url}"
            )
            return None
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = e
                logger.warning(
                    f"Attempt {attempt + 1} failed to generate semantic context: {str(e)}"
                )
                await asyncio.sleep(2)  # Wait before retry

        if not semantic_context:
            logger.error(
                f"Failed to generate semantic context after {max_retries} attempts: {str(last_error)}"
            )
        ...
        return ProcessedChunk(
            ...
            metadata={
                ...
                "source_type": "priesvezi_docs"
                if "priesvezi.lt" in url
                else "pola_docs",
                "language": "lt",
                "links": links,
            },
        ...
        logger.error(
            f"Error processing chunk {chunk_number} from {url}: {str(e)}", exc_info=True
        )
        return None


async def save_markdown_to_file(
    url: str, markdown: str, base_dir: str = "data/markdown"
) -> str:
...
async def process_and_store_document(url: PolaURL, markdown: str, db: DatabaseClients):
    ...
    try:
        ...
        if url_tracker.is_url_processed(normalized_url):
            ...
            logger.info(
                f"Skipping already processed URL {normalized_url} with status: {status['status']}"
            )
            return
        ...
        if is_priesvezi:
            ...
            if content_start == -1:
                logger.info(
                    f"No title marker found in {normalized_url}, skipping document"
                )
                return
        ...
        if existing_doc_response.data and len(existing_doc_response.data) > 0:
            logger.info(
                f"Document {normalized_url} already exists with hash {content_hash[:8]}, skipping"
            )
            return
        ...
        document_data = {
            ...
            "language": "lt",
            "source_type": "priesvezi_docs"
            if "priesvezi.lt" in normalized_url
            else "pola_docs",
            "metadata": {
        ...
        doc_insert_response = (
            db.supabase.table("documents").insert(document_data).execute()
        )
        ...
        chunks_insert_response = (
            db.supabase.table("document_chunks").insert(chunk_records).execute()
        )
        if not chunks_insert_response.data:
            raise Exception(
                f"Failed to insert chunks for document {doc_id} into Supabase"
            )

        logger.info(
            f"Successfully processed {normalized_url}: {len(processed_chunks)} chunks"
        )
...
async def crawl_parallel(urls: List[PolaURL], max_concurrent: int = 5):
    ...
    unprocessed_urls = [
        url for url in urls if not url_tracker.is_url_processed(url.url.strip("/"))
    ]
    ...
    logger.info(
        f"Processing {len(unprocessed_urls)} new URLs out of {len(urls)} total URLs"
    )
    ...
    try:
                    ...
                    for attempt in range(3):  # Try 3 times
                        try:
                            ...
                            # Success, exit retry loop
                                ...
                                logger.warning(
                                    f"Attempt {attempt + 1} failed for {pola_url.url}: {result.error_message}"
                                )
                                await asyncio.sleep(2)  # Wait before retry
                        except Exception as e:
                            logger.warning(
                                f"Attempt {attempt + 1} error for {pola_url.url}: {str(e)}"
                            )
        ...
        for i in range(0, len(unprocessed_urls), batch_size):
            ...
            logger.info(
                f"Processing batch {i//batch_size + 1} of {(len(unprocessed_urls) + batch_size - 1)//batch_size}"
            )
...
class SitemapProcessor:
    ...
    def __init__(
        self, sitemap_urls: List[str], storage_file: str = "sitemap_state.json"
    ):
        ...
        try:
            if os.path.exists(self.storage_file):
                ...
                with open(self.storage_file, "r", encoding="utf-8") as f:
                    state = json.load(f)
                print(
                    f"Debug: Loaded state with {len(state.get('sitemaps', {}))} sitemaps"
                )
                return state
            else:
                print(
                    f"Debug: No existing sitemap state file, creating new at {self.storage_file}"
                )
    ...
    def _save_state(self, state: Dict[str, Any] = None):
        ...
        try:
            ...
            with open(self.storage_file, "w", encoding="utf-8") as f:
                json.dump(state, f, indent=2, default=str)
            print(
                f"Debug: Successfully saved state with {len(state.get('sitemaps', {}))} sitemaps"
            )
        except Exception as e:
            ...
            try:
                ...
                print(
                    f"Debug: Failed to save state even after creating directory: {write_error}"
                )

    def update_sitemap_state(
        self, sitemap_url: str, urls_found: int, success: bool = True
    ):
                        ...
                        try:
                            ...
                            for url_elem in root.findall(f".//{namespace}url"):
                                ...
                                if last_mod_elem is not None and last_mod_elem.text:
                                    try:
                                        last_mod = datetime.fromisoformat(
                                            last_mod_elem.text.replace("Z", "+00:00")
                                        )
                                        print(
                                            f"Debug: Last modified date for {loc}: {last_mod}"
                                        )
                                    except ValueError as e:
                                        print(
                                            f"Debug: Invalid lastmod format for {loc}: {e}"
                                        )
                                        last_mod = datetime.now(timezone.utc)

                                urls.append(
                                    PolaURL(
                                        url=loc, image_count=0, last_modified=last_mod
                                    )
                                )
                                ...
                                if sitemap_urls_count % 10 == 0:
                                    print(
                                        f"Debug: Processed {sitemap_urls_count} URLs from {sitemap_url}"
                                    )

                            print(
                                f"Debug: Completed processing {sitemap_url}, found {sitemap_urls_count} URLs"
                            )
                            sitemap_processor.update_sitemap_state(
                                sitemap_url, sitemap_urls_count, True
                            )
                            logger.info(
                                f"Successfully parsed sitemap {sitemap_url}: {sitemap_urls_count} URLs found"
                            )
